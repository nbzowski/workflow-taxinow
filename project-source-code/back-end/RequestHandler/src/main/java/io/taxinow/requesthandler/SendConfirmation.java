    ...
    public static void main(String[] args) {
        try (ZeebeClient client = ZeebeClientFactory.getZeebeClient()) {
            client.newWorker().jobType(JOB_TYPE).handler((jobClient, job) -> {
                ...
                client.newPublishMessageCommand().messageName("receive-driver-conf").correlationKey(userSessionID).variables(variablesMap).send().exceptionally( throwable -> { throw new RuntimeException("Could not publish message", throwable); });;
