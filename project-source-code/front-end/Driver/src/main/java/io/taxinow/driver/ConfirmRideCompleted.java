...
public class ConfirmRideCompleted {


    private static final Logger LOG = LogManager.getLogger(ConfirmRideCompleted.class);

    public static void main(String[] args) {
        try (ZeebeClient client = ZeebeClientFactory.getZeebeClient()) {
            client.newWorker().jobType("confirm-ride-completion").handler((jobClient, job) -> { // service-task-id must match the task type in Camunda!
                ...
                client.newPublishMessageCommand().messageName("diver-msg-send-completion-request").correlationKey(userSessionID).send().exceptionally(throwable -> {
